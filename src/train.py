...
if __name__ == "__main__":
    ...
    DATA_DIR = "C:\\Users\\Semyon\\YandexLyceum\\project\\yandex-ml-2025\\data\\human_poses_data"
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"Train - Loss: {train_loss:.4f}, Acc: {train_accuracy:.2f}%, Precision: {train_precision:.4f}, Recall: {train_recall:.4f}, F1: {train_f1:.4f}")
        if valid_loader:
            print(f"Valid - Loss: {val_loss:.4f}, Acc: {val_accuracy:.2f}%, Precision: {val_precision:.4f}, Recall: {val_recall:.4f}, F1: {val_f1:.4f}")
